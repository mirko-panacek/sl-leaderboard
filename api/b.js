module.exports = (req, res) => {
    ...
    const entries = raw && raw !== "Updating~0"
        ? raw.split("|").map((e, i) => {
            const module.exports = (req, res) => {
    const raw = req.query.d || "";
    const entries = raw && raw !== "Updating~0"
        ? raw.split("|").map((e, i) => {
            const parts = e.split("~");
    ...
    const isUpdating = raw === "Updating~0";
...
h1{font-size:28px;font-weight:700;color:#fff;letter-spacing:3px;text-transform:uppercase;
   border-bottom:2px solid #fc0;padding-bottom:8px;margin-bottom:12px;text-align:center}
table{width:100%;border-collapse:collapse;font-size:22px}
th{color:#555;font-size:14px;font-weight:600;letter-spacing:1px;text-transform:uppercase;
   text-align:left;padding:4px 6px;border-bottom:1px solid #222}

// Breeders leaderboard endpoint
// URL: /api/b?d=Name1~42|Name2~38|...
module.exports = (req, res) => {
    ...
    const rows = entries.length
        ? entries.map(e => `
            <tr>
                <td class="r">${esc(e.rank)}</td>
                <td class="n">${esc(e.name)}</td>
                <td class="l">${e.level}</td>
            </tr>`).join("")
...
h1{font-size:16px;color:#fff;border-bottom:2px solid #fc0;padding-bottom:5px;
...
.r{color:#888;width:20px}
.n{color:#fc0}
.l{color:#0f0;text-align:right;font-weight:bold}
